...
const QUESTIONS_TPL = [
  ...
  { id: 3, text: "O que é gastroplastia endoscópica e como funciona?", intent: "Descoberta", priority: "media" },
...
function getQuestions(cidade) {
  return QUESTIONS_TPL.map(q => ({ ...q, text: q.text.replace("{cidade}", cidade || "sua cidade") }));
}
...
export default function GeoMonitor({ medicos = [], s = {} }) {
  ...
  const [modo, setModo] = useState("medico"); // "medico" | "plataforma"
  ...
  function calcOverall(mId) {
    const vals = PLATFORMS.flatMap(pl => QUESTIONS_TPL.map(q => getScore(mId, q.id, pl.id) ?? 0));
    return Math.round((vals.reduce((a, b) => a + b, 0) / (vals.length * 3)) * 100);
  }
    ...
    <div style={{ color: text }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20, flexWrap: "wrap", gap: 12 }}>
        <div>
          ...
          <div style={{ fontSize: 13, color: muted, marginTop: 2 }}>10 perguntas · {medicos.length} médicos</div>
        </div>
        {/* Modo toggle */}
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
              <thead>
                <tr style={{ borderBottom: `1px solid ${border}` }}>
                  <th style={{ padding: "10px 12px", textAlign: "left", color: muted, fontWeight: 500, whiteSpace: "nowrap" }}>Médico</th>
                          ...
                          <td key={q.id} style={{ padding: "4px", textAlign: "center" }}>
                            <select value={current ?? ""} onChange={e => setScore(m.id, q.id, pl.id, Number(e.target.value))}
                              style={{
                                ...
                                color: opt ? opt.color : muted, borderRadius: 4, padding: "3px", fontSize: 11, cursor: "pointer", textAlign: "center"
                              }}>
                              <option value="">—</option>
