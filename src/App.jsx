  ...
  return <div style={{minHeight:"100vh",background:s.bg,color:s.text,fontFamily:"'DM Sans',sans-serif"}}>
    ...
    <div style={{background:"linear-gradient(to right,#0A1628,#070D1A)",borderBottom:`1px solid ${s.border}`,padding:"16px 20px",display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:12,position:"sticky",top:0,zIndex:100}}>
      ...
      <nav style={{display:"flex",gap:4,flexWrap:"wrap"}}>
        {[{id:"dashboard",label:"Dashboard",icon:"📊"},{id:"entrada",label:"Registrar",icon:"✏️"},{id:"historico",label:"Histórico",icon:"📅"},{id:"medicos",label:"Médicos",icon:"👨‍⚕️"},{id:"geo",label:"Monitor GEO",icon:"🤖"},{id:"perfil",label:"Perfil Digital",icon:"🔧"},{id:"impacto",label:"Impacto",icon:"📊"},{id:"media",label:"Media Monitor",icon:"📰"}
].map(tab=>(
    ...
    <div style={{padding:"20px",maxWidth:1180,margin:"0 auto"}}>
      ...
      {/* Seletor de médico compacto para dashboard e histórico */}
      {(view==="dashboard"||view==="historico")&&<div style={{marginBottom:16}}>
        <select value={medicoSel?.id||""} onChange={e=>setMedicoSel(medicos.find(m=>m.id===e.target.value))} style={{background:s.card,border:`1px solid ${s.border2}`,color:s.text,padding:"8px 12px",borderRadius:8,fontSize:13,minWidth:220}}>
          {medicos.map(m=><option key={m.id} value={m.id}>{m.nome}{m.cidade?` — ${m.cidade}`:""}</option>)}
        </select>
      </div>}
      ...
      {/* MONITOR GEO */}
      {view==="geo"&&<div><GeoMonitor medicos={medicos} s={s}/></div>}
      ...
      {/* MEDIA MONITOR */}
      {view==="media"&&<div><MediaMonitor s={s}/></div>}
